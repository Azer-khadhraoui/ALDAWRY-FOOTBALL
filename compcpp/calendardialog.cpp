#include "../compheaders/calendardialog.h"
...
{
    QSqlQuery query;
    query.prepare("SELECT MATCH_DATETIME FROM match WHERE id_competition = :comp_id "
                  "AND EXTRACT(YEAR FROM MATCH_DATETIME) = :year AND EXTRACT(MONTH FROM MATCH_DATETIME) = :month");
