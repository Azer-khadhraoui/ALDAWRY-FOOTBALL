...
void MatchesDialog::populateMatches()
{
    ...
    if (m_isSingleDay) {
        query.prepare("SELECT m.MATCH_DATETIME, "
                      ...
                      "m.scorea, m.scoreb "
                      "FROM match m "
                      "WHERE m.id_competition = :comp_id AND TRUNC(MATCH_DATETIME) = TO_DATE(:date, 'YYYY-MM-DD')");
        ...
        query.prepare("SELECT m.MATCH_DATETIME, "
                      ...
                      "m.scorea, m.scoreb "
                      "FROM match m "
                      "WHERE m.id_competition = :comp_id AND EXTRACT(YEAR FROM m.MATCH_DATETIME) = :year AND EXTRACT(MONTH FROM m.MATCH_DATETIME) = :month");
        query.bindValue(":comp_id", m_comp_id);
